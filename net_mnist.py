...
class FastCapsLayer(nn.Module):
    ...
    def forward(self, caps_output):     #这里的caps_output就是PrimaryCaps中得到的out,Size(N,32*28*28,8)=(128,25088,8)
        ...
        reconst_targets1 = reconst_targets1.expand(reconst_targets1.size(0),6,6,10)
        reconst_targets1 = reconst_targets1.permute(0, 3, 1, 2).contiguous()
        #print(reconst_targets1.size())      #torch.Size([128, 6, 6, 10])
        reconst_targets2 = u_predict.view(u_predict.size(0),6,6,-1)
...
class PrimaryCapsLayer(nn.Module):
    ...
    def forward(self, input):
        ...
        out = out.view(out.size(0), -1, out.size(4))    #view函数类似于resize     (N,1152,8)
...
class CapsNet(nn.Module):
    def __init__(self, routing_iterations, n_classes=10):   #n_classes输出胶囊数量，10个，对应10个类别
        ...
        self.conv1 = nn.Conv2d(1, 256, kernel_size=9, stride=1) #卷积核是9*9的
        self.primaryCaps = PrimaryCapsLayer(256, 1, 256, kernel_size=9, stride=2)  # 256个输入通道，32个输出胶囊，8是每个胶囊的维度，每个胶囊尺寸是6*6
        self.num_primaryCaps = 1 * 6 * 6   #卷积层胶囊数量
...
class ConvReconstructionNet(nn.Module):
    def __init__(self, n_dim=16, n_classes=10):
        ...
        self.deconv2 = nn.ConvTranspose2d(16, 1, 4, stride=2,padding=1)
